...
# Resolve the interpreter, repo root, and scripts dir portably so the
# benchmark CLI tests run on any machine (not just the original author's).
# The medrack package is installed in the active environment; the repo root
# is the directory that contains the `medrack` package.
PY = Path(sys.executable)
MEDRACK = Path(__file__).resolve().parents[2]
SCRIPTS_DIR = Path(sys.executable).parent  # Scripts/ (Windows) or bin/ (POSIX)

# Make the medrack package importable when running from a source checkout.
...
def test_cli_via_medrack_benchmark_in_help():
    ...
    medrack_bin = SCRIPTS_DIR / ("medrack.exe" if os.name == "nt" else "medrack")
    r = subprocess.run(
