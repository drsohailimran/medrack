...
def _force_utf8_output() -> None:
    """Make stdout/stderr encode UTF-8 so glyphs like ✓/✗ print on Windows.

    Windows consoles default to a legacy code page (e.g. cp1252) which
    cannot encode the status glyphs the CLI prints, raising
    ``UnicodeEncodeError``. Reconfiguring to UTF-8 is a no-op on platforms
    that already use it (e.g. Linux/macOS) and is wrapped defensively so it
    never breaks captured or redirected streams (e.g. under pytest).
    """
